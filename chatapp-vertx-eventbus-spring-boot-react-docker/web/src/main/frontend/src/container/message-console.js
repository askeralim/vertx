...
class MessageConsole extends Component {
  ...
  keyDown(component, event){
     if(event.keyCode == 13 && event.shiftKey == false){
       ...
       const fromId = component.props.loginToken.get('id');
       const fromName = component.props.loginToken.get('name')
       const toId = component.props.activeChatRoom.get('id');
       const toName = component.props.activeChatRoom.get('nickName');
       let loginToken = JSON.parse(localStorage.getItem('loginToken'));
       component.props.dispatch(sendChatMessageAction(loginToken.token, event.target.value, fromId, toId, component.props.activeChatRoom.get('type'),fromName, toName ));
