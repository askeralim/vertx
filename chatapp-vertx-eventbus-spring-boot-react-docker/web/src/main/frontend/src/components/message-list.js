...
class MessageList extends Component{
  renderList(){
      ...
      return messageList.map(message => {
        if(message.get('fromId') == this.props.loginToken.get('id'))
          return (
...
function mapStateToProps({chatRoomList, messageMap, activeChatRoom, loginToken}){// picking weather as state.weather
  return {chatRoomList, messageMap, activeChatRoom, loginToken}//  {weather} is same as {weather:weather}
}
